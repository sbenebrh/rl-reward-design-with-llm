...
def main(condition, shorter, num_seeds, model, thresholds):
    for threshold in thresholds:
        ...
        for seed in range(num_seeds):
            ...
            means.append(mean)
            stds.append(std)
        print("threshold: ",threshold, "mean: ", np.mean(means), "std: ", np.mean(stds))

def evaluate_lm(condition, shorter, thresholds):
    for threshold in thresholds:
        partial_path = f"{condition}_{threshold}"
        if shorter:
            evaluate_lm_responses_shorter(partial_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["run_lr_agent", "evaluate_llm"],
        help="Choose whether to run the LR agent or evaluate the LLM",
    )
    parser.add_argument(
        "--condition",
        type=str,
    ...
    if args.operation == "run_lr_agent":
        main(
            args.condition,
            args.shorter,
            args.num_seeds,
    ...
    elif args.operation == "evaluate_llm":
        evaluate_lm(
            args.condition,
            args.shorter,
            thresholds=thresholds_by_condition[args.condition],
