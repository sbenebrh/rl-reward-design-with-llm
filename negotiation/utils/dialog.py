...
class Dialog(object):
    ...
    def is_agreement(self, out):
        """
        Check whether the end of the conversation has been reached
        """
        return len(out) == 1 and (out[0] == "agree" or out[0] == "disagree")
    ...
    def write(self, writer, logger, agree_disagree, forced=False):
        """
        Produces an utterance and saves necessary meta information
        """
        # produce an utterance
        out = writer.write()
        if agree_disagree == 1  and not(len(out) == 1 and out[0] == "<selection>"):
                out = ["<selection>"]
        if not writer.human:
    ...
    def run(self, ctxs, logger, update, forced=False, training=True):
        ...
        try:
            ...
            while True:
                # produce an utterance
                out = self.write(writer, logger,agree_disagree, forced=forced)
